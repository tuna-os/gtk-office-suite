...
- **Flakiness:** GUI tests are inherently more flaky than unit tests. Dogtail uses timing-dependent operations (wait for widget to appear). Test scripts must include explicit waits and retries.
  - **Wait for a condition, never a fixed time, before sending keys.** Raw keys go to whatever has the keyboard *when they arrive*. `LettersSmoke`'s word-count journey slept 1.5 s after New Document and typed. On the first launch of a run, the new tab took longer than that, so the welcome page was still up and the window's find bar captured the keys: the find bar read "kbrownfo" and the document "2 words" (run 36201572079, attempt 1). Start a Letters document with `new_letters_document()`, which returns once the page view has the keyboard. After typing, wait for the result (`wait_for_node`), not a sleep.
  - **A `gapplication action` is not queued behind keys.** Keys travel through X while an action arrives over D-Bus immediately, so an action can run before earlier keystrokes land. Wait for their effect (the text, the selection) before sending the action.
