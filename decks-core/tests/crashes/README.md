# Retained crash inputs — Decks

Every file in this directory is an input that once made
`decks_core::odp::read` panic. `tests/malformed_inputs.rs` replays all of
them on every pull request, so a fixed crash stays fixed.

This directory being empty means no ODP crash has been found yet — not that
...
2. Commit the minimized bytes here with a name that says what is wrong,
   e.g. `truncated-central-directory.odp`.
