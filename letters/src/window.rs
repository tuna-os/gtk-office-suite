...
impl LettersWindow {
    pub fn new(app: &adw::Application, settings: gio::Settings) -> Self {
        ...
        register_formatting_actions(&tab_view, app);
...
/// the close guard).
pub(crate) fn save_page(page: &adw::TabPage) -> bool {
    let child = page.child();
...
pub(crate) fn clear_tab_autosave(child: &gtk::Widget) {
    if let Some(td) = tab_data_get(child) {
        let _ = td.0.borrow().autosave_slot.clear();
    }
}
